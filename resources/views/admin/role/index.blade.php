...
<script type="text/javascript">
    ...
    function del(id) {
        layer.confirm('确认删除此角色?', {icon: 3, title: '提示'}, function (index) {
            //do something
            $.getJSON("{{url('admin/role/delete')}}"+'/'+id, function (res) {
                if (1 == res.code) {
    ...
    $("#postform").click(function(){
        ...
        //写入库
        $.post("{{url('admin/role/giveAccess')}}", {'type' : 'give', 'id' : id, 'rule' : NodeString, '_token' : _token}, function(res){
            layer.close(index);
            if(1 == res.code){
                layer.alert(res.msg, {title: '友情提示', icon: 1, closeBtn: 0}, function(){
                    initTable();
